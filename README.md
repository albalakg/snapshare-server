# TODO


* Add Facebook & Instagram links - V
* Add a note to the event's date with "בחרו תאריך ושעה מדוייקת של תחילת האירוע" - V
* Add a shareable link with readonly to the guests - V
* Add multiple qr cards available - V
* Add notification if the event is about to start in 5 days & 1 day and the status is not "ready" - V
* Add note for deployment link - V
* Change that the event will be available 8 hours before the start time - V
* מציג האירוע אינו פעיל - V?
* לינק לשיתוף אלבום שתשלח למפיקה - V?
* ⁠לסדר את התמונות של האורחים או התמונות האישיות שישמר - V?
* Show QR in Album - ?
* Add multiple galleries - ?

