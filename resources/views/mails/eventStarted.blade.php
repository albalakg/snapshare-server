...
@section('content')
    ...
    <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0;">
        ...
        <p><strong>תאריך:</strong> {{ $event->starts_at->format('dd/mm/YYYY') }}</p>
        <p><strong>שעת התחלה:</strong> {{ $event->starts_at->format('H:i') }}</p>
        <p><strong>תאריך סיום:</strong> {{ $event->finished_at->format('dd/mm/YYYY H:i') }}</p>
    </div>
